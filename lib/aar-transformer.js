...
class AarTransformer {
   ...
   *
   * Uses constructor injection for a appc logger instance
   *
   * @param {Object} logger Instance of an appc logger
   */
  constructor(logger) {
    if (!logger) {
      throw new Error('AarTransformer requires a logger instance to be passed to its constructor');
    }
    this.logger = logger;
  }
